...
class ATJoint(ATComponent):
    ...
    async def perform_configurate(self, config: ATComponentConfig, auth_token: str = None, *args, **kwargs) -> bool:
        ...
        at_blackboad_item = config.items.get('at_blackboad')
        at_blackboad = AT_BLACKBOARD
        if at_blackboad_item is not None:
            at_blackboad = at_blackboad_item.data

        return self.create(
            at_solver=at_solver, 
            at_temporal_solver=at_temporal_solver, 
            at_simulation=at_simulation, 
            at_blackboad=at_blackboad, 
